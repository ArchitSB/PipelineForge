      ...
      <div className="node-library__list">
        {NODES.map((n) => (
          <div
            ...
            draggable
            onDragStart={(e) => startDrag(e, n.type)}
            title={n.desc}
          >
            <div className="node-library__item-accent" style={{ background: n.accent }} />
            <span className="material-symbols-outlined" style={{ fontSize: '16px', color: n.accent }}>{n.icon}</span>
            <div className="node-library__item-info">
              <span className="text-label-md" style={{ color: 'var(--color-on-surface)' }}>{n.label}</span>
              <span style={{ fontSize: '10px', color: 'var(--color-on-surface-variant)' }}>{n.desc}</span>
            </div>
          </div>
        ))}
      </div>
